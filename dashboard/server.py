...
try:
    from actions.device_tracker import register_or_update_real_device, mark_device_offline
except Exception:
    register_or_update_real_device = None
    mark_device_offline = None
...
<p>Connecting to JARVIS…</p>
        ...
        # ── Real Phone Live GPS & Battery Telemetry ───────────────────────────

        @app.post("/api/telemetry")
        async def telemetry_ep(req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            try:
                body = await req.json()
                dev_id = str(body.get("device_id") or f"Phone-{self._ip.split('.')[-1]}").upper()
                name = str(body.get("device_name") or "Primary Mobile Device")
                ...
                if register_or_update_real_device and (lat != 0.0 or lon != 0.0):
                    register_or_update_real_device(
                        device_id=dev_id,
                        name=name,
                        lat=lat,
                        lon=lon,
            ...
            try:
                while True:
                    ...
                    if msg_type == "command":
                        ...
                        name = data.get("device_name") or f"Phone ({tok[:4]})"
