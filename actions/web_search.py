...
def web_search(
    ...
    # Safe string/list parser for comparison aspects
    raw_aspect = params.get("aspect", "general")
    if isinstance(raw_aspect, list):
        aspect = ", ".join(str(a) for a in raw_aspect)
    else:
        aspect = str(raw_aspect).strip() or "general"
