    ...
    if _OS == "Windows":
        try:
            ...
            if hasattr(devices, "EndpointVolume"):
                ...
                return
            elif hasattr(devices, "device") and hasattr(devices.device, "Activate"):
            ...
            # Safe Windows PowerShell Volume Fallback
            try:
                ps_script = f"""
                [Audio.Volume]::SetMasterVolume({value})
                """
            except Exception:
                pass
    elif _OS == "Darwin":
...
def computer_settings(
    ...
    # ── Volume Control (Accepts "volume", "volume_set", "set_volume", and handles "60%") ──
    if action in ("volume_set", "volume", "set_volume", "sound", "master_volume"):
        try:
            # Safely extract digits even if Gemini sends "60%", "level 60", etc.
            val_match = re.search(r"\d+", str(value or description or "50"))
            val_int = int(val_match.group()) if val_match else 50
            volume_set(val_int)
            return f"Master volume set to {val_int}%."
        except Exception as e:
